...
const deleteTweet = async (req:Request, res:Response) => {
    try {
        ...
        if (!tweet) return res.status(404).json({ error: "Well Well look who is tryin to delete tweet without creating a one" });
...
const allTweets = async (req:Request, res:Response) => {
    try {
...
export { createTweet, getTweet, deleteTweet, allTweets };
